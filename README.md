...
اگر `--lot` ندهید، حجم از روی `MAX_LOSS_USD` و فاصلهٔ استاپ محاسبه و رو به پایین
گرد می‌شود؛ اگر استاپ برای سقف ریسک زیادی پهن باشد، `place` به‌جای اضافه‌ریسک‌کردن،
سفارش را رد می‌کند.
...
(۲۷ مه تا ۴ سپتامبر ۲۰۲۶) بک‌تست شده‌اند. قوانین پروتکل (سشن، ۵ ترید در روز،
...
|---|---|
| حداکثر حجم | `0.01` لات |
| حداکثر پوزیشن/سفارش هم‌زمان | `1` |
| حداکثر ضرر هر ترید | `5$` (طلا: `9$`) |
| هدف سود هر ترید | `15$` (طلا: `27$`) |
| حداکثر ترید در روز | `5` |
| استاپ روزانه | ضرر `15$-` یا سود `50$+` |
| سشن | فقط نیویورک |
